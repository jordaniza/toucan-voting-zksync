...
contract PriceFeed is ILayerZeroPriceFeed, OwnableUpgradeable, Proxied {
    ...
    mapping(uint32 => Price) internal _defaultModelPrice;
    ...
    function _estimateFeeWithDefaultModel(
        ...
        return (
            (remoteFee * remotePrice.priceRatio) / PRICE_RATIO_DENOMINATOR,
            remotePrice.priceRatio
        );
    }
    ...
    function _estimateFeeWithArbitrumModel(
        ...
        uint256 gasFee = (_gas +
            _arbitrumPriceExt.gasPerL2Tx +
            gasForL1CallData +
            gasForL2CallData) * arbitrumPrice.gasPriceInUnit;

        return (
            (gasFee * arbitrumPrice.priceRatio) / PRICE_RATIO_DENOMINATOR,
            arbitrumPrice.priceRatio
        );
    }
}
