...
abstract contract DVNAdapterBase is Worker, ILayerZeroDVN {
    ...
    mapping(address => mapping(uint32 => bytes32)) public receiveLibs;
    ...
    function setReceiveLibs(
        ReceiveLibParam[] calldata _params
    ...
    // ========================= Internal =========================
    function _getAndAssertReceiveLib(
        address _sendLib,
        uint32 _dstEid
    ) internal view returns (bytes32 lib) {
    ...
    function _encodeEmpty() internal pure returns (bytes memory) {
        return
            DVNAdapterMessageCodec.encode(
                bytes32(0),
                new bytes(DVNAdapterMessageCodec.PACKET_HEADER_SIZE),
                bytes32(0)
            );
    }
