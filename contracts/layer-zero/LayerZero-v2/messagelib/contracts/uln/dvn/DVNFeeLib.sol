...
contract DVNFeeLib is Ownable, IDVNFeeLib {
    ...
    function getFeeOnSend(
        ...
        (uint256 fee, , , uint128 nativePriceUSD) = ILayerZeroPriceFeed(_params.priceFeed)
            .estimateFeeOnSend(_params.dstEid, callDataSize, _dstConfig.gas);
    ...
    function getFee(
        ...
        uint256 callDataSize = _getCallDataSize(_params.quorum);
        (uint256 fee, , , uint128 nativePriceUSD) = ILayerZeroPriceFeed(_params.priceFeed)
            .estimateFeeByEid(_params.dstEid, callDataSize, _dstConfig.gas);
        return
            _applyPremium(
                fee,
                _dstConfig.multiplierBps,
