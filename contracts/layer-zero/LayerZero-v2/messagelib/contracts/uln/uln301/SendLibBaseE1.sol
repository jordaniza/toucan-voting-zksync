...
abstract contract SendLibBaseE1 is SendLibBase, AddressSizeConfig, IMessageLibE1 {
    ...
    function send(
        ...
        (bytes memory encodedPacket, uint256 totalNativeFee) = _payWorkers(
            _sender,
            _dstEid,
            _path,
            _message,
        ...
        bool payInLzToken = _lzTokenPaymentAddress != address(0x0) &&
            address(lzToken) != address(0x0);
        ...
        if (lzTokenFee > 0) {
            ...
            if (_lzTokenPaymentAddress != sender)
                revert LZ_MessageLib_LzTokenPaymentAddressMustBeSender();
    ...
    function _payWorkers(
        ...
        (bytes memory executorOptions, WorkerOptions[] memory verificationOptions) = _splitOptions(
            _options
        );
