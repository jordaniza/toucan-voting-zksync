...
abstract contract ReceiveUlnBase is UlnBase {
    ...
    function _verify(
        ...
        hashLookup[keccak256(_packetHeader)][_payloadHash][msg.sender] = Verification(
            true,
            _confirmations
        );
    ...
    function _checkVerifiable(
        ...
        for (uint8 i = 0; i < _config.optionalDVNCount; ++i) {
            if (
                _verified(_config.optionalDVNs[i], _headerHash, _payloadHash, _config.confirmations)
            ) {
