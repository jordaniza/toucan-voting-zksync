...
abstract contract SendUlnBase is UlnBase {
    ...
    function _assignJobs(
        ...
        (bytes[] memory optionsArray, uint8[] memory dvnIds) = DVNOptions.groupDVNOptionsByIdx(
            dvnOptions
        );
    ...
    function _quoteDVNs(
        ...
        // if options is not empty, it must be dvn options
        bytes memory dvnOptions = _options.length == 0 ? bytes("") : _options[0].options;
        (bytes[] memory optionsArray, uint8[] memory dvnIndices) = DVNOptions.groupDVNOptionsByIdx(
            dvnOptions
        );
