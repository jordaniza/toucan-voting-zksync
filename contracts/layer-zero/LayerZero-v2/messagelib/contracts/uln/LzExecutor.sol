...
interface IReceiveUlnView {
    ...
    ) external view returns (VerificationState);
}
...
contract LzExecutor is OwnableUpgradeable, EndpointV2ViewUpgradeable, Proxied {
    ...
    function commitAndExecute(
        ...
        ExecutionState executionState = executable(
        ...
        if (executionState != ExecutionState.Executable) {
            address receiveLib = receiveUln302 == address(0x0)
                ? _receiveLib
            ...
            bytes32 payloadHash = keccak256(
                abi.encodePacked(_lzReceiveParam.guid, _lzReceiveParam.message)
            );

            address receiveLibView = receiveLibToView[receiveLib];
            if (receiveLibView == address(0x0)) revert LzExecutor_ReceiveLibViewNotSet();

            VerificationState verificationState = IReceiveUlnView(receiveLibView).verifiable(
                packetHeader,
                payloadHash
            );
