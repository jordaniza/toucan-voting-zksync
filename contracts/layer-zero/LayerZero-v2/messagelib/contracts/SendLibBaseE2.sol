...
abstract contract SendLibBaseE2 is SendLibBase, ERC165, ISendLib {
    ...
    constructor(
        ...
        SendLibBase(
            _endpoint,
            ILayerZeroEndpointV2(_endpoint).eid(),
            _treasuryGasLimit,
            _treasuryNativeFeeCap
        )
    {}
    ...
    function withdrawLzTokenFee(address _lzToken, address _to, uint256 _amount) external {
        ...
        if (ILayerZeroEndpointV2(endpoint).nativeToken() == _lzToken)
            revert LZ_MessageLib_CannotWithdrawAltToken();
