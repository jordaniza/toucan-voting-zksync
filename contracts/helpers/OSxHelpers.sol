...
import {MockDAOFactory, PluginSetupRef} from "contracts/helpers/osx/MockDAOFactory.sol";
import {MockPluginSetupProcessor} from "contracts/helpers/osx/MockPSP.sol";
...
function wrapGrantRevokeRoot(
    ...
    actions[0] = IDAO.Action({
        ...
        data: abi.encodeCall(
            _dao.grant,
            (address(_dao), _psp, _dao.ROOT_PERMISSION_ID())
        )
    });
...
function _mockPluginSettings(
    ...
    MockDAOFactory.PluginSettings[]
        memory settings = new MockDAOFactory.PluginSettings[](1);
...
function _mockPrepareUninstallationParams(
    ...
    return
        MockPluginSetupProcessor.PrepareUninstallationParams(
            _mockPluginSetupRef(),
            payload
        );
}
